...
try: # errors end up in error.txt
    ...
    # Since this should be run after the schedule for the season has been generated,
    # FBGM's "schedule" already has games for us
    GID = max([x["gid"] for x in export["schedule"]])+1
    # Figure out CCG week; just the last week the game generated + 1
    # The game calls them "days" because all of the other GM games are bolted onto
    # BBGM's framework, and basketball isn't played in weeks.
    ...
    # need the latest one.
    for conference in export["gameAttributes"]["confs"][-1]["value"]:
